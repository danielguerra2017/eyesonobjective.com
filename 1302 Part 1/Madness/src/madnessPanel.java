...
public class madnessPanel extends JPanel
{
	...
	public madnessPanel()
	{
		...
		String[] marvel = {"Nubs", "Useless Belgium", "Bald Darkman", "Speratic Greatest"};
		...
		Object[] optionsTwo = {"Furry Dachshund Hearder", "14 Year Old Boy Wonder", "Chairborne Rager", "Lonely Saiga Man"};
				
		int superheroPick = JOptionPane.showOptionDialog(frame,
				"Choose the character you wish to play as!",
				"Madness",
				JOptionPane.YES_NO_OPTION,
				JOptionPane.QUESTION_MESSAGE,
				null,     //do not use a custom Icon
		...
		if (superheroPick == 0)
		{
			yourChar = "Furry Dachshund Hearder";
		}
		
			else if (superheroPick == 1)
			{
				yourChar = "14 Year Old Boy Wonder";
			}
		
				else if (superheroPick == 2)
				{
					yourChar = "Chairborne Rager";
				}
		
					else
					{
						yourChar = "Lonely Saiga Man";
					}
		...
		while (health > dead && badHealth > dead)
		{
			...
			if (difficulty == 0)
			{
				...
				if (superheroPick == 0)
				{
					String[] yourMoves = {"used their bowcaster on", "summoned a hoard of dachshunds to attack", "used their home planet mating call on", "gave their dachshunds a treat to attack"};
				}
				
					else if (superheroPick == 1)
					{
						String[] yourMoves = {"used an overpowered pistol on", "lectured about his loved life to", "made food eatin noises in the ear of", "flew a little bird and attacked"};
					}
				
						else if (superheroPick == 2)
						{
							String[] yourMoves = {"raged about latency issues to", "raged about weapons to", "ordered 2 pizzas and didn't share with", "raged about his perfect game to"};
						}
				
							else
							{
								String[] yourMoves = {"pulled out his saiga and shot", "flew an attack chopper and attacked", "talked about having a girlfriend once to", "had crappy internet and blamed it on"};
							}
